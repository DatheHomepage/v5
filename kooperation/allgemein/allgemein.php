<div class="titel">- Allgemeine Kooperation -</div>
<div class="teaser">
    <br>
    <b>Übersicht über aktuelle Kooperationen der Dathe-Oberschule (Stand: 07/2015)</b>
    <br>
    <br> Wir kooperieren, in Anlehnung an unser Schulprofil, mit verschiedensten Partnern,
    <br> um den Unterricht und das Schulleben mit Erfahrungen und Erlebnissen aus dem Alltag,
    <br> der Umwelt, der Kultur, der Politik anzureichern.
    <br>
    <br> Die folgende Übersicht gibt eine unvollständige Auswahl wieder und
    <br> wird durch viele Einzelaktionen der jeweiligen Lehrkräfte in ihrem Unterricht und bei Projekten ständig erweitert.
    <br>
    <br>
    <b>Unsere Schule pflegt folgende Kooperationen</b>
    <br>
    <table>
        <tr>
            <th width="35%">Mit wem?</th>
            <th width="55%">Was?</th>
            <th width="10%">Ansprechpartner/in</td>
        </tr>
        <tr>
            <td>
                Grundschulen im Bezirk
                <br> Friedrichshain-Kreuzberg,
                <br> insbesondere
                <br>Thalia-Grundschule,
                <br> Modersohn-Grundschule,
                <br> auch Grzimek-Grundschule und
                <br>34.Grundschule Scharnweberstr.</br>
            </td>
            <td>
                <li>Zus.arbeit bei Übergang von Grund- zur Oberschule</li>
                <li>Gemeinsame Projekte/ Veranstaltungen</li>
                <li>Gegenseitige Infos Besonderheiten der Schulform</li>
            </td>
            <td>Frau Gruska</td>
        </tr>
        <tr>
            <td>Partnerschulen im Ausland</td>

            <td>Poprad - Slowakei
                <br> Narbonné - Frankreich
                <br>
            </td>

            <td>FB Erdkunde
                <br>
                <br> FB Fremdsprachen
            </td>
        </tr>
        <tr>
            <td>ALBA</td>
            <td>Unterricht und AG Basketball
                <br>
                <br>
            </td>
            <td>Herr Zauber </td>
        </tr>
        <tr>
            <td>Blumberger Mühle</td>
            <td>Schülerfahrten und Unterrichtprojekte
                <br>zur Waldpädagogik und gesunden
                <br>Ernährung in den Klasse 5 und 6
                <br>
                <br>
            </td>
            <td>Frau Firgens</td>
        </tr>
        <tr>
            <td>BIZ Berufsinformationszentrum</td>
            <td>
                <li>Sprechstunden in der Schule</li>
                <li>Workshops und Veranstaltungen in den Jahrgängen 9, 10, 11 und 12</li>
                <br>
                <br>
            </td>
            <td>
                Berufsberater
                <br> Herr Meise
            </td>
        </tr>
        <tr>
            <td>Datenschutz-Verein</td>

            <td>Workshops in Klasse 7
                <br>
                <br>
            </td>

            <td>Frau Gutte</td>
        </tr>
        <tr>
            <td>FEZ</td>
            <td>Wetterprojekt Klasse 7
                <br>
                <br>
            </td>
            <td>Herr Reinke-Nobbe</td>
        </tr>
        <tr>
            <td>
                FU Berlin
                <br> Biologie - Didaktik
            </td>
            <td>
                Hospitation und Unterrichtspraktikum von Lehramtstudenten unter Nutzung unserer Biologiestation
                <br>
                <br>
            </td>
            <td>Herr Reinke-Nobbe</td>
        </tr>
        <tr>
            <td>FU Berlin
                <br> Studenten machen Schule</td>

            <td>Workshop zur Vorbereitung der
                <br>Präsentationsprüfungen im MSA
                <br>und Abitur
                <br>
                <br>
            </td>

            <td>Frau Gruska</td>

        </tr>

        <tr>

            <td>HU Berlin u.a.</td>

            <td>Klimamessprojekt zum Raumklima
                <br>
                <br>
            </td>

            <td>Frau Schulze</td>

        </tr>

        <tr>

            <td>HU Berlin - PSE
                <br> professional school of education
            </td>

            <td>Kooperation zur Begleitung des Enrichmentunterrichts zur Begabungsförderung
                <br>
                <br>
            </td>

            <td>Frau Blazy</td>

        </tr>

        <tr>

            <td>Klimafrühstück KATE e.V.</td>

            <td>Unterrichtsprojekt im Rahmen des Enrichmentkurses Nachhaltige Ernährung
                <br>
                <br>
            </td>
            <td>Frau Dressler</td>

        </tr>

        <tr>

            <td>Naturkundemuseum</td>

            <td>Arbeitsgemeinschaften im Rahmen der Begabungsförderung
                <br>
                <br>
            </td>

            <td>Frau Weidemann</td>

        </tr>

        <tr>

            <td>Ökowerk</td>

            <td>Unterrichtsprojekte im Wahlpflichtunterricht und den Biologie-Kursen, z.B. zur Ökologie
                <br>
                <br>
            </td>

            <td>Herr Reinke-Nobbe</td>

        </tr>

        <tr>

            <td>Polizei
                <br> Abschnitt 51
            </td>

            <td>Gemeinsame Veranstaltungen zur gewalt- und Suchtprävention in allen Jahrgangsstufen
                <br>
                <br>
            </td>

            <td>Herr Ambros</td>

        </tr>

        <tr>

            <td>Theater der Schulen</td>

            <td>Theaterbesuche
                <br>
                <br>
            </td>

            <td>Frau Tarbiat</td>

        </tr>

        <tr>

            <td>Tierpark</td>

            <td>
                <li>Tierpatenschaft über die Silberäffchen</li>
                <li>Tierparkprojekt im Jahrgang 7</li>
                <li>Kontakt zum Förderverein</li>
                </ul>
            </td>

            <td>Frau Weidemann</td>

        </tr>

        <tr>

            <td>Wolke auf Reisen e.V.</td>

            <td>Vielfältige Einzelprojekte und Schülerfahrten aus dem bereich der Erlebnispädagogik Unterstützung der Gesamtschülervertretung
                <br>
                <br>
            </td>

            <td>Herr Christian Herold</td>

        </tr>
        <tr>

            <td>Intellego-Förderschule</td>

            <td>
                zur Unterstützung der Schüler/innen
                <br>
                <li>alle Fächer/ alle Klassen</li>
                <li>mit Berlinpass kostenlos</li>
                <li>sonst 5€ /45min</li>
            </td>

            <td>über Klassenleitung</td>

        </tr>
        <tr>

            <td>Freunde für AMPO e.V.</td>

            <td>
                Hilfsprojekt für eine Kinder-und Jugendeinrichtungen in Burkina Faso
            </td>

            <td>Frau Bartz</td>

        </tr>
        <tr>

            <td>Unabhängiges Institut für Umweltfragen UfU e.V.</td>

            <td>
                Foto-Workshop mit Seminarkurs
                <br>Umwelt baut Brücken</br>
            </td>

            <td>Herr Fiedler</td>

        </tr>
    </table>

</div>
</body>

</html>